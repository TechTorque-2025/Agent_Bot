...
# Create StructuredTool instances for async functions
all_tools = [
    StructuredTool.from_function(
        coroutine=check_appointment_slots_tool,
        name="check_appointment_slots_tool",
        description="Checks the available appointment slots for a given date (YYYY-MM-DD) and service_type (e.g., 'Oil Change', 'Diagnostics'). Use this tool ONLY when the user asks for available times or scheduling."
    ),
    StructuredTool.from_function(
        coroutine=get_user_active_services_tool,
        name="get_user_active_services_tool",
        description="Retrieves a list of all IN_PROGRESS services and projects for the current user. Use this tool when the user asks for the status of their vehicle or project."
    ),
    StructuredTool.from_function(
        coroutine=get_last_work_log_tool,
        name="get_last_work_log_tool",
        description="Retrieves the most recent time log and technician note for a specific service or project ID. The service_id must be provided by the user or extracted from the conversation history."
    )
]